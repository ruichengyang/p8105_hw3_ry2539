...
ggplot(Zip_Code_Rent_df, aes(x = Date, y = rental_price)) + geom_point(aes(color = CountyName)) + facet_wrap(~ZipCode)
...
ggplot(full_nhane_joint_data_df, aes(x = age, fill = sex)) + geom_histogram(binwidth = 5, alpha = 0.7) +
...
ggplot(total_MIMS_df, aes(x = age, y = Total_MIMS, color = sex)) + geom_point(alpha = 0.4) +
  geom_smooth(method = "loess", se = FALSE) + facet_grid(education~sex)
...
ggplot(time_minute_average_df, aes(x = hour, y = Mean_MIMS, color = sex)) + geom_point(alpha = 0.4) +
  geom_smooth(method = "loess", se = FALSE) + facet_wrap(~education)
