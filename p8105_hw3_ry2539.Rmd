...
#Use pivot longer to see rental price for each year and month for each zip code#
...
Year_Borough_df = Year_1_pivot_longer_df |> drop_na(`Rental Price`) |> group_by(Year, CountyName) |>
  summarize(mean_rental_price = mean(`Rental Price`, na.rm = TRUE)) |> arrange(CountyName)

Year_Borough_df

ggplot(Year_Borough_df, aes(x = Year, y = mean_rental_price)) + geom_point(aes(color = CountyName), alpha = 0.5) +
  geom_smooth(se = FALSE) + facet_grid(.~CountyName) + labs(title = "Mean Rental Price Across the Years for All Boroughs", x = "Year", y = "Mean Rental Price", color = "Boroughs")
...
Zip_Code_Rent_df = Zip_Code_Rent_Year_df |> group_by(Year, ZipCode, CountyName, Month) |> summarize(rental_price = `Rental Price`, na.rm = TRUE)
...
every_rent_zip_code = ggplot(Zip_Code_Rent_df, aes(x = Date, y = rental_price)) + geom_point(aes(color = ZipCode)) + facet_wrap(~CountyName) + labs(title = "Rental Price Within every Zip Code for All Available Years", x = "Date", y = "Rental Price", color = "Zip Codes")

every_rent_zip_code

#separate data based on zip code and export it to plot#

p = ggplot(Zip_Code_Rent_df, aes(x = Date, y = rental_price)) + geom_point(aes(color = CountyName), alpha = 0.5) + facet_wrap(~ZipCode) + labs(title = "Rental Price Within every Zip Code for All Available Years", x = "Date", y = "Rental Price", color = "Corresponding Borough")
...
ggplot(Twenty_Twenty_Three_Zip_Code_rent_df, aes(x = ZipCode, y = mean_rental_price)) + geom_point(aes(color = CountyName), alpha = 1) + labs(title = "Mean Rental Price in 2023 and Zip Code", x = "Zip Code", y = "Mean Rental Price", color = "Counties")
...
table_sex_education_distribution
```
...
ggplot(full_nhane_joint_data_df, aes(x = age, fill = sex)) + geom_histogram(binwidth = 4, alpha = 0.5) +
  facet_grid(education~sex) + labs(title = "Distribution of Ages Across Sexes and Education", x = "Age", y = "Count of People", color = "Sex")
...
ggplot(total_MIMS_df, aes(x = age, y = Total_MIMS, color = sex)) + geom_point(alpha = 0.5) +
  geom_smooth(se = FALSE) + facet_grid(education~sex)+ labs(title = "Total MIMS Varied for All Ages", x = "Age", y = "Total MIMS", color = "Sex")
...
#Plot out the average MIMS vs the hour of the day for all sexes and education levels#
